...
This is the vendored copy that ships with the examples so each demo runs standalone
from this directory; it mirrors ../client/twin.py (plus the `with_port` helper below).
"""
import json
import urllib.error
...
__all__ = ["Twin", "Agent", "TwinError", "with_port"]


def with_port(base_url, port=None):
    """Return `base_url` with its port set to `port` (no-op when `port` is None).

    Accepts a URL with or without a scheme ("http://host:8000", "host", "host:9000")
    and defaults a missing scheme to http, so the demos can take a plain --url plus an
    optional --port override.
    """
    if port is None:
        return base_url
    u = urlparse(base_url if "://" in base_url else "http://" + base_url)
    return str(urlunparse(u._replace(netloc=f"{u.hostname or 'localhost'}:{int(port)}")))
