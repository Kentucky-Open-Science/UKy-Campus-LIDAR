#!/usr/bin/env python3
"""Drive a ROBOT autonomously by vision: first-person camera -> YOLO -> controls.

The robot spawns into the SHARED twin world and navigates using only what its camera
sees (a YOLO model picks out obstacles; it steers around them and cruises otherwise).
It's a differential-drive bot, so `steer` pivots it. Everyone connected — other scripts
and any browser — sees it drive.
...
Start the server with the camera feed on, then run this:
    ...
    python examples/robot_demo.py                    # this script (any machine: --url HOST:8000)
    python examples/robot_demo.py --port 9000 --model yolo26x.pt
    # open http://localhost:8000/ to watch

Runs standalone from this directory (twin.py + yolo_drive.py) plus third-party deps
(ultralytics, pillow). Press Ctrl-C any time — the robot is despawned from the server.
"""
import argparse
...
TYPE, COLOR, START = "robot", 0x4aa05a, [-127.0, None, 1224.1]  # x,z on the map; None height auto-snaps onto the terrain
