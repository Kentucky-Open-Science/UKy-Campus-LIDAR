...
vendored `twin.py` client in this directory — so it runs anywhere, the twin lives on its
own server, and the drone it spawns is part of the SHARED world: anyone else connected
(another script or a browser viewing http://<server>:8000/) sees this drone fly and
crash in real time.
...
Then run this from any machine:
    ...
    python examples/drone_demo.py --port 9000 --owner alice

Open http://<server>:8000/ in a browser while it runs to watch the drone in 3-D.
Runs standalone from this directory (only twin.py, pure stdlib). Press Ctrl-C any time —
the drone is despawned from the server.
"""
import argparse
import time
...
def fly(twin, drone):
    """The whole flight: square circuit, then dive into the nearest building."""
    s = drone.state()
    sx, sy, sz = s["position"]
    utm = s.get("utm")
    ...
    # fly a square circuit at hover altitude ----------------------------------
    ...
    # drop onto the nearest building and fly into it ---------------------------
    ...
    print(f"\ndone — drone at rest, speed {final['speed']} m/s; despawning "
          f"(other clients saw the whole flight).")
...
def main():
    ...
    print(f"connected to twin at {url}  (sim {meta['hz']} Hz, "
